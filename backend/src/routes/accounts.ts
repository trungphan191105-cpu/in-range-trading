...
router.post('/', validateRequest(createAccountSchema), (req: AuthRequest, res) => {
  ...
  db.prepare(`
    INSERT INTO accounts (id, student_id, name, prop_firm, phase, currency, initial_balance, current_balance, max_daily_loss_pct, max_total_drawdown_pct, profit_target_pct, color, logo_url)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, studentId, name, prop_firm || null, phase || 'funded', currency || 'USD', Number(initial_balance), Number(initial_balance), max_daily_loss_pct || 5, max_total_drawdown_pct || 10, profit_target_pct || null, color || '#38bdf8', logo_url || null);
...
router.put('/:id', validateRequest(updateAccountSchema), (req: AuthRequest, res) => {
  ...
  const { name, prop_firm, phase, currency, current_balance, max_daily_loss_pct, max_total_drawdown_pct, profit_target_pct, status, color, logo_url } = req.body;
  db.prepare(`UPDATE accounts SET
    name = ?, prop_firm = ?, phase = ?, currency = ?, current_balance = ?,
    max_daily_loss_pct = ?, max_total_drawdown_pct = ?, profit_target_pct = ?,
    status = ?, color = ?, logo_url = ?, updated_at = datetime('now')
    WHERE id = ?
  `).run(
    name ?? acc.name, prop_firm ?? acc.prop_firm, phase ?? acc.phase,
    ...
    status ?? acc.status, color ?? acc.color, logo_url !== undefined ? logo_url : acc.logo_url, req.params.id
  );
